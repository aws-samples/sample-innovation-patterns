...
### 1.3a Verify Prepare Stacks Are Deployed
...
3. For each prepare stack, check status:
   ...
   ```
4. If **any** prepare stack does not exist or is not in a `*_COMPLETE` state, STOP with:

   ```
   Prerequisite infrastructure must be deployed before deploying the solution.
   Run `/ipa-prepare` to deploy one-time prerequisites.

   Example: /ipa-prepare
   ```

   Do NOT auto-invoke `/ipa-prepare`.

**If `scripts/prepare.mk` is missing**: "A solution must be composed before deployment. Run `/ipa-compose` to generate deployment artifacts."
...
**If not deployed**: "Security stack `{APP_NAMESPACE}-{APP_ENV}-security` is not deployed. Run `/ipa-compose` to configure security and compose infrastructure."
