...
|----------|--------|-----------|-------------|
| `BUILDER_ROLE_ARN` | Stack output or builder input | Always | Builder execution role ARN |
| `CODEBUILD_ROLE_ARN` | Stack output or builder input | Always (managed policy) / Optional (existing role) | CodeBuild execution role ARN |
| `MANAGED_POLICY` | Builder input | Managed policy path only | Policy name or ARN used |
| `SECURITY_STACK_NAME` | Computed | Always | `{IPA_NAMESPACE}-{IPA_ENV}-security` |
| `LOG_BUCKET_NAME` | Stack output | Always | `{IPA_NAMESPACE}-{IPA_ENV}-logs-{AWS_ACCOUNT_ID}-{AWS_REGION}` |
| `KMS_KEY_ARN` | Builder input | Optional | KMS key ARN for encryption at rest |
...
Check if `.env` contains security variables (`BUILDER_ROLE_ARN` or `SECURITY_STACK_NAME`):
...
Compute the stack name: `{IPA_NAMESPACE}-{IPA_ENV}-security`

Run:
```bash
aws cloudformation describe-stacks \
  --stack-name {IPA_NAMESPACE}-{IPA_ENV}-security \
  --region {AWS_REGION} \
  --profile {AWS_PROFILE} \
...
3. If skipped (empty input): note that `CODEBUILD_ROLE_ARN` will be omitted from `.env`.
...
BUILDER_ROLE_ARN={from stack output BuilderRoleArn}
CODEBUILD_ROLE_ARN={from stack output CodeBuildRoleArn}
MANAGED_POLICY={builder-provided policy name or ARN}
SECURITY_STACK_NAME={IPA_NAMESPACE}-{IPA_ENV}-security
LOG_BUCKET_NAME={from stack output LogBucketName}
```
...
# IPA Security Configuration
# Generated by /ipa.security — local only, do not commit
BUILDER_ROLE_ARN={builder-provided ARN}
SECURITY_STACK_NAME={IPA_NAMESPACE}-{IPA_ENV}-security
LOG_BUCKET_NAME={from stack output LogBucketName}
```

Include `CODEBUILD_ROLE_ARN={builder-provided ARN}` only if the builder provided a CodeBuild role ARN (not skipped).

#### KMS Key (Both Paths):

If the builder provided a KMS key ARN, add to the security block:
```
KMS_KEY_ARN={builder-provided KMS key ARN}
```
...
```
Written to .env:
  BUILDER_ROLE_ARN={value}
  CODEBUILD_ROLE_ARN={value}
  MANAGED_POLICY={value}
  SECURITY_STACK_NAME={value}
  LOG_BUCKET_NAME={value}
```
...
Read current security variables from `.env`:
- `BUILDER_ROLE_ARN`
- `CODEBUILD_ROLE_ARN` (may be absent)
- `MANAGED_POLICY` (may be absent — indicates existing-role path)
- `SECURITY_STACK_NAME`
- `LOG_BUCKET_NAME`
- `KMS_KEY_ARN` (may be absent)

Verify the CloudFormation stack exists and is in a `*_COMPLETE` state (not `ROLLBACK_COMPLETE`):
```bash
aws cloudformation describe-stacks \
...
│ Builder Role ARN     │ arn:aws:iam::123:role/myproject-dev-builder     │
│ CodeBuild Role ARN   │ arn:aws:iam::123:role/myproject-dev-codebuild   │
│ Security Stack       │ myproject-dev-security                          │
│ Log Bucket           │ myproject-dev-logs-123456789012-us-east-1       │
│ KMS Key              │ (AWS-managed, default)                          │
└──────────────────────┴─────────────────────────────────────────────────┘
```

Infer the current path: if `MANAGED_POLICY` is set → "Managed Policy"; otherwise → "Existing Role ARNs".
...
Display: "The log bucket name `{bucket_name}` already exists in S3 (bucket names are globally unique). This usually means another project or account owns this name. Consider changing your `IPA_NAMESPACE` via `/ipa.init` to generate a different bucket name."
