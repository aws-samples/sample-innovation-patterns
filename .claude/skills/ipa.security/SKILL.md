...
| `SECURITY_STACK_NAME` | Computed | Always | `{APP_NAMESPACE}-{APP_ENV}-security` |
| `LOG_BUCKET_NAME` | Stack output | Always | `{APP_NAMESPACE}-{APP_ENV}-logs-{AWS_ACCOUNT_ID}-{AWS_REGION}` |
...
SECURITY_STACK_NAME={APP_NAMESPACE}-{APP_ENV}-security
