...
The queue stack is a consolidated deploy tier for event-driven background processing. It provisions an SQS standard queue with an optional dead-letter queue, a container-packaged worker Lambda triggered through an EventSourceMapping, and a feature-flagged DynamoDB table for job tracking. The stack is included via `/ipa-compose` when the application needs asynchronous processing, and deploys before the backend tier so that queue outputs can be wired into backend parameters.
...
- 30-day log retention on the worker log group.
