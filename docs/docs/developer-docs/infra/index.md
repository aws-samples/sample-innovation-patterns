...
CloudFormation templates, Terraform modules, and container definitions for all IPA infrastructure. Templates are organized into **tier stacks** that consolidate related AWS services and **prepare stacks** that wrap single prerequisite services. A generated security stack provides shared IAM roles.

When `APP_IAC=terraform` is set in `.env`, IPA generates Makefile targets that use Terraform CLI (`terraform init/apply/destroy`) instead of AWS CloudFormation CLI. Both engines produce functionally equivalent infrastructure — the choice is made at `/ipa-init` time and affects only the deployment mechanism, not the architecture.
...
graph TD
    subgraph "Prepare Stacks (one-time)"
        Logs["logs.yml<br/>(S3 log bucket)"]
        Security["security.yml<br/>(IAM roles)"]
    ...
    Logs -->|LogBucketDomainName| Frontend
...
│   ├── logs/
│   │   └── logs.yml                 # Prepare: Centralized S3 log bucket
│   ├── codecommit/
...
| **Examples** | backend, frontend, queue | logs, cognito, ecr, codecommit, codepipeline |
...
The generated security stack (`generated/iam.yml`) is a special case — it is produced by the `/ipa-security` skill and deployed as a prepare stack, providing shared IAM execution roles. The centralized log bucket (`logs/logs.yml`) is a separate prepare stack deployed via `/ipa-prepare`.
...
|-------|----------|----------|-----------|
| `/ipa-security` | `generated/iam.yml` | — | prepare |
| `/ipa-stack-logs` | `logs/logs.yml` | `prepare.mk` | prepare |
...
To validate a CloudFormation template before deploying:

```bash
aws cloudformation validate-template --template-body file://infra/cfn/backend/backend.yml
```

To validate all Terraform modules:
...
| `infra/cfn/logs/logs.yml` | Centralized S3 log bucket (prepare stack) |
| `infra/cfn/tfstate/tfstate.yml` | Terraform state backend (S3 + DynamoDB lock table) |
| `infra/cfn/generated/iam.yml` | Shared IAM roles (generated by /ipa-security) |
| `infra/cfn/*/CLAUDE.md` | Per-template companion docs — authoritative for modification workflows |
| `infra/tf/*/main.tf` | Terraform module — resource definitions |
| `infra/tf/*/variables.tf` | Terraform module — input variables with validation |
| `infra/tf/*/outputs.tf` | Terraform module — exported values |
| `infra/tf/*/versions.tf` | Terraform module — provider/backend configuration |
...
- **Security stack is generated** — `generated/iam.yml` is produced by `/ipa-security`, not hand-edited. Manual edits are overwritten on the next skill invocation.
...
- [Terraform IaC Support](./terraform.md) — Terraform module architecture and wiring patterns
