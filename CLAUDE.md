...
- Markdown (Claude Code skill format) — no executable code + Claude Code skill framework, existing `utils/ipa_utils/` execution layer, GNU Make (009-compose-stacks)
- Filesystem — reads from `.claude/skills/`, `.env`, `infra/cfn/`, `patterns/`; writes to `.claude/skills/ipa.compose/`, `scripts/`, `docs/infra/` (009-compose-stacks)
...
## Recent Changes
- 009-compose-stacks: Added Markdown (Claude Code skill format) — no executable code + Claude Code skill framework, existing `utils/ipa_utils/` execution layer, GNU Make
