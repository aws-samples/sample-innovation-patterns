...
- **Split-plane model — local pod reads the real deployed DynamoDB table** — the data plane (DynamoDB) is stood up by the existing `/ipa-compose → /ipa-prepare → /ipa-deploy` flow against the backend tier; only the compute plane runs locally in k3d. They converge by **table-name convergence** (`PynamodbUtil.env_table_name` and `backend.yml` both build `{APP_NAMESPACE}_{APP_ENV}_passengers`) plus host-mounted `~/.aws` credentials. The `Tiltfile` injects the `APP_NAMESPACE`/`APP_ENV`/`AWS_REGION`/`APP_REGION` convergence trio from the repo-root `.env` via `dotenv()` → `helm(set=[...])`, so the local pod resolves the same table the backend tier created — no CFN-output wiring and no `helm` compose mode (`/ipa-compose` is untouched). Tilt verifies credentials (`aws sts get-caller-identity`) before the pod starts, so it never comes up with dead credentials.
- **Cloud-ready Helm chart as the customer handoff artifact** — Deployment + Service + ServiceAccount + helpers + NOTES, production-shaped by default (auth on, one replica, resource limits) and re-pointed per environment by values overlays. The AWS-credential mount is gated by an `aws.credentials.mode` toggle (`none` for cloud IRSA, `direct` host-mount, `initCopy` default — an init-container copies host creds into an emptyDir for non-root pods). The ServiceAccount is IRSA-annotatable and image/env/resources are values-overridable, so the chart targets a real cluster with `helm` alone — no IPA, no Claude Code, no chart edits. An annotated, **non-deployed** `infra/k8s/envs/eks/` overlay (template only, placeholder account ID `123456789012`) shows exactly what a cloud overlay looks like.
- **`/ipa-k8s-help` guidance skill** — an optional Claude Code helper that is the interactive counterpart to `make doctor`. Local scope runs a 7-step check-and-instruct flow (tools, credentials, table-name convergence, table-exists, cluster state, guided run, symptom→remediation triage); EKS scope walks the `eks/` overlay and validates cloud pre-conditions, referencing `infra/k8s/envs/eks/CLAUDE.md` as the single source of guidance. It checks and instructs only — never auto-installs and never applies to a cluster. Its description disambiguates it from `/ipa-help` (the lifecycle state inspector).

### Changed
- **Sidebar collapses to an icon rail instead of fully hiding** — `web-client` left nav switched from `collapsible="offcanvas"` to `collapsible="icon"`, so toggling now shrinks the sidebar to a 3rem square-icon strip (Slack/Linear/VS Code pattern) rather than sliding it off-screen. The brand row collapses to just its icon, group labels and the version badge hide, and all nav items gained hover tooltips so the bare icons stay identifiable when collapsed.

### Documentation
- New `developer-docs/infra/kubernetes.md` (chart/Tiltfile/ctlptl/Makefile internals, mirroring `terraform.md`) and `developer-docs/infra/kubernetes-tools.md` (optional non-IPA community CLIs — k9s, kubectx/kubens, kftray, stern — install links only, not checked by `make doctor`).
- New builder guides `guides/kubernetes-local-development.md` (turnkey local-loop how-to) and `guides/kubernetes-path-to-production.md` (chart handoff). Both, plus `infra/k8s/README.md`, carry a **Future Work** section recording the deferred cloud-cluster path (CFN/TF cluster provisioning, ECR wiring, IRSA role creation, a CodePipeline `helm upgrade --install` stage, Local IRSA, and ingress templates) — none built this round. Agent-context `CLAUDE.md` files added across `infra/containers/rest-k8s/`, `infra/k8s/`, the chart, and `eks/`.
...
- **`.graphifyignore` for scoped knowledge-graph indexing** — added repo-root `.graphifyignore` using gitignore-style patterns (`*` then `!app-lib/`, `!app-lib/**`) to scope `/graphify` indexing to the Python backend library. Keeps generated artifacts, infra templates, web-client build output, and skill markdown out of the graph so queries return signal from application code instead of scaffolding.
