"""Unit tests for app_lib.common.util.observability module."""

import json
from unittest.mock import MagicMock, patch

from app_lib.common.util.observability import (
    log_bedrock_usage,
    log_exception,
...
class TestLogException:
    ...
    def test_log_exception_emits_json(self, capsys):
        ...
        with patch("app_lib.common.util.observability.logger") as mock_logger:
...
class TestPublishMetric:
    ...
    def test_publish_metric_calls_boto3(self):
        ...
        with patch(
            "app_lib.common.util.observability._get_cw_client",
            ...
            assert {"Name": "Env", "Value": "dev"} in metric_data["Dimensions"]
...
class TestLogBedrockUsage:
    ...
    def test_log_bedrock_usage_publishes_metrics(self):
        ...
        with (
            patch("app_lib.common.util.observability.publish_metric") as mock_publish,
            patch("app_lib.common.util.observability.logger"),
        ):
            response = {
            ...
            assert calls[0][1]["dimensions"] == {"ModelId": "anthropic.claude-3-sonnet"}
...
class TestMetricNamespace:
    ...
    def test_metric_namespace_from_env(self):
        ...
        with patch.dict(
            ...
            import app_lib.common.util.observability as obs
