...
Used by the exception middleware in common/app.py.
...
def log_bedrock_usage(
    ...
    if tokens["input_tokens"]:
        publish_metric("BedrockInputTokens", tokens["input_tokens"], dimensions=dims)
    if tokens["output_tokens"]:
        publish_metric("BedrockOutputTokens", tokens["output_tokens"], dimensions=dims)
    entry = {
        "level": "INFO",
        "bedrock_usage": True,
        "model_id": model_id,
