...
.PHONY: deploy deploy-cognito deploy-ddb-passengers deploy-fn deploy-apigwv2 deploy-s3 deploy-cf teardown teardown-cf teardown-s3 teardown-apigwv2 teardown-fn teardown-ddb-passengers teardown-cognito

deploy: deploy-cognito deploy-ddb-passengers deploy-fn deploy-apigwv2 deploy-s3 deploy-cf
...
deploy-fn: deploy-cognito deploy-ddb-passengers
...
deploy-apigwv2: deploy-fn deploy-cognito
...
deploy-cf: deploy-s3
	$(eval BUCKET_DOMAIN_NAME := $(shell aws cloudformation describe-stacks \
		--stack-name $(APP_NAMESPACE)-$(APP_ENV)-s3 \
		--query 'Stacks[0].Outputs[?OutputKey==`BucketDomainName`].OutputValue' \
		--output text \
		$(if $(AWS_PROFILE),--profile $(AWS_PROFILE),) $(if $(AWS_REGION),--region $(AWS_REGION),)))
	$(eval BUCKET_ARN := $(shell aws cloudformation describe-stacks \
		--stack-name $(APP_NAMESPACE)-$(APP_ENV)-s3 \
	...
	aws cloudformation deploy \
		...
		--parameter-overrides Namespace=$(APP_NAMESPACE) Environment=$(APP_ENV) \
			S3BucketDomainName=$(BUCKET_DOMAIN_NAME) \
			S3BucketArn=$(BUCKET_ARN) S3BucketName=$(BUCKET_NAME) \
			LogBucketDomainName=$(LOG_BUCKET_NAME).s3.amazonaws.com \
		--no-fail-on-empty-changeset

# === TEARDOWN (reverse order) ===

teardown: teardown-cf teardown-s3 teardown-apigwv2 teardown-fn teardown-ddb-passengers teardown-cognito
